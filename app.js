...
veryOldInventors = inventors.filter((ele) => {
   if(ele.year > 1499 && ele.year <1600) {
    veryOldInventors.push(ele); 
...
inventorNames = inventors.map((ele) => {

    return {first:ele.first,last:ele.last}
...
inventorNamedAda = inventors.find(ele => 
ele.first ==='Ada');
...
let firstLast = [];
firstLast = people.map(ele => {
    const [last, first] = ele.split(', '); // Split by ", "
    return `${first} ${last}`; // Return in "First Last" format
  });
...
isAdultPresent = devs.some(ele => currentYear - ele.year >= 18);
...
// Complete the exercise in the space below:
commentById = comments.find(elle => elle.id === 823423);
